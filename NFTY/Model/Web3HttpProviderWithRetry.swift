...
public struct Web3HttpProviderWithRetry: Web3Provider {
  ...
  let session = UrlTaskThrottle(
    queue:DispatchQueue(label: "Web3HttpProviderWithRetry.serialQueue",qos:.userInitiated),
    deadline:DispatchTimeInterval.milliseconds(10),
  ...
  public func send<Params, Result>(request: RPCRequest<Params>, response: @escaping Web3ResponseCompletion<Result>) {
    ...
    self.session.enqueue(with: req) { data, urlResponse, error in
      ...
      guard (status != 429) else {
        ...
        self.session.queue.asyncAfter(deadline: .now()+(random_retry_ms / 1000.0)) {
